    ...
    <index-lay-out @getLostThings="getLostThings">
      <template slot="tableShows">
        ...
        <el-table max-height="598" :data="tableBody" style="width: 100%">
          ...
          <el-table-column prop="lostTime" label="丢失时间" width="110">
          </el-table-column>
    ...
    >
      <el-form v-model="editData" label-width="100px">
        ...
        <el-form-item label="丢失时间">
          <el-input v-model="editData.lostTime"></el-input>
        </el-form-item>
...
import { GetAllLost, EditLostInfo, DeleteLostInfo } from '@/api/api'
export default {
  data() {
    return {
      tableData: [], // 表格数据
  ...
  methods: {
    ...
    handleEdit(index, row) {
      ...
      this.editData.lostTime = row.lostTime
    ...
    editSure() {
      // 调用接口
      EditLostInfo(this.editData).then((res) => {
    ...
    handleDelete(index, row) {
      ...
      this.$confirm(
        ...
        .then(() => {
          // 调用删除接口
          DeleteLostInfo(deleteData).then((res) => {
            if (res.data.code == 1) {
              this.$message({
                type: 'success',
                message: '删除成功!'
